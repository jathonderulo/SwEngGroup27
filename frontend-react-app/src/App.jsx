  ...
  const handleMessageSubmit = async (newMessage) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Server responded with an error:', response.status, errorDetails);
        // Optionally, update the UI to show the error
        setMessages(prevMessages => [...prevMessages, { id: Date.now(), sender: "System", text: `Error: ${errorDetails}` }]);
        return; // Prevent further execution
      }
  
      const data = await response.json();
      // Add the response message to the chat window
      setMessages(prevMessages => [...prevMessages, { id: data.id, sender: "Server", text: data.message }]);
    } catch (error) {
      console.error('Error fetching data:', error);
      // Display error message in the chat window
      setMessages(prevMessages => [...prevMessages, { id: Date.now(), sender: "System", text: 'Error: Unable to fetch data from the server' }]);
    }
  };
